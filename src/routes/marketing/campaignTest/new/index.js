...
function toNearest30min() {
  ...
  const dateTime = moment().add(remainder, "minutes").toDate();
  // const dateTime = moment(start).add(remainder, "minutes").toISOString();
...
class marketing_campaign_new extends Component {
  ...
  handleSubmit() {
    ...
    this.props.history.push("/app/marketing/campaign");
  ...
  render() {
    ...
    const { classes, campaignForm } = this.props;
    return (
      <React.Fragment>
        <Helmet title="New Campaign" metaDesc="Marketing Campaigns Creation" />
        {campaignForm.loading && <RctSectionLoader />}
...
const mapStateToProps = ({ sessionState, marketingState }) => {
  ...
  return { loggedInUser, campaignForm };
};
