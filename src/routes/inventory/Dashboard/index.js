...
class Index extends Component {
  ...
  _SelectGradeExterior = async e => {
    ...
    const ExteriorGrade = await api.get(
      `products/specificVariantExterior/${e.id}`
    );

    const InteriorGrade = await api.get(
      `products/specificVariantInterior/${e.id}`
    );

    const ProductOptions = await api.get(
      `products/specificGradeProductOption/${e.id}`
    );
          ...
          <div className="row">
            ...
            <div className="col-md-9">
              <ProfileTabs loading={false}>
                <div label="Mega Menu">
                  <MegaMenu _SetModelID={this._SetModelID} />
                </div>
                ...
                <div label="Exterior">
                  <Exterior Exterior={this.state.Exterior} />
                </div>

                <div label="Interior">
                  <Interior Interior={this.state.Interior} />
                </div>
