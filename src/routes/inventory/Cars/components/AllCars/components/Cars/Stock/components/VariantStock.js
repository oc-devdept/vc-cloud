...
const validateForm = (keys) => {
    ...
    let errorForm = ''
    let Reject = true
    if(name == ""){Reject = false;errorForm="Please fill up the name of your order"}
    if(trackingId == 0){Reject = false;errorForm="Please input your tracking Id"}
    if(ship_count == 0){Reject = false;errorForm="Please state your quantity"}
    if(status == ""){Reject = false;errorForm="Please select a status"}
...
const index = ({id}) => {
    ...
    const submitOrder = async(form) => {
        ...
        if(result[0]){
            try {
                await api.post(`/stockhistories/createStockOrder`, {data: {form, id: id}});
                const result = await api.get(`/productvariantvalues/${id}/stockhistory`);
                setOrderHistory(() => result.data)
    ...
    const submitEditOrder = async(form) => {
        ...
        if(result[0]){
            try {
                await api.patch(`/stockhistories/editStockOrder`, {data: {form, id: id}});
                const result = await api.get(`/productvariantvalues/${id}/stockhistory`);
                setOrderHistory(() => result.data)
                NotificationManager.success('Stock order form submitted successfully');
            } catch (e) {
        ...
        },
        {
            ...
            options: {
                ...
                customBodyRender: (rowData, rowState) => {
                    return (
                        <Edit
                          onClick={() => editOrder(rowState.rowIndex)}
                        />
                    );
                }
            }
...
const Form = ({submitOrder, restartOrder}) => {
    ...
    const _setFormFieldStatus = (e) => {
        ...
        setFormFields(Form => ({...Form, status:e.target.value, estimate_time_delivery: estimateDate}))
    }
    ...
    return (
        <div className="d-flex flex-column " style={{flex:1}}>
            {ShowForm? 
                <div>
                    <div className="d-flex flex-row align-items-center" style={{flex:1, width:'100%'}}>
                        ...
                        {/* <Input
                            ...
                            _HandleProduct={_HandleFields}
                        /> */}
                        <div className="d-flex flex-column flex-fill" style={{marginRight: 15}}>
                    ...
                    <div className="d-flex flex-row justify-content-end" style={{marginTop: 20}}>
                        <Button
                        ...
                        <Button
                            _Function={() => {setShowForm(() => false); restartOrder()}}
                            product={''}
                            files={''}
                            title={'CLOSE'}
                        />
