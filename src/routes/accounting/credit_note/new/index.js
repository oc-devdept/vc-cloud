...
// Actions
import { postSingleCreditNote } from "Ducks/accounting/credit";
...
class acct_new_payment extends Component {
  ...
  _submitPayment = e => {
    ...
    const r = window.confirm(
      `Click OK to confirm the credit note payment for ${this.state.invoice.customerName}`
    );
    if (r == true) {
      let creditNote = {
        creditNote: this.state.invoice,
  ...
  render() {
    ...
    return loading ? (
      ...
      // companyList
      <React.Fragment>
        <Helmet title="Payment" metaDesc="Everyday Payment Management" />
